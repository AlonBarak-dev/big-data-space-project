...
router.get('/', getCached, async function (req, res, next) {
    try{
        ...
        const mongo_data = await c.toArray();
        if(!mongo_data.length){
            res.json({});
            return;
        }

        const created_source = await axios.post(`https://bigml.io/andromeda/source?${AUTH}`, {data: JSON.stringify(mongo_data)});
