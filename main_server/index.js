...
app.get(
  "/get_event_list_date_by_star/:from/:to/:star_name",
  async (req, res) => {
    const from = req.params.from;
    const to = req.params.to;
    ...
    const filtered_events_in_range = events_in_range.filter(
      (event_in_range) => event_in_range.star == star_name
    );
...
app.get(
  "/get_event_list_date_by_notfac/:from/:to/:notfac",
  async (req, res) => {
    const from = req.params.from;
    const to = req.params.to;
  ...
  return new Promise((resolve, reject) => {
    redisClient.get(key, async (err, value) => {
      ...
      if (value !== null) {
        ...
        // TODO fit for elastic when Eyal is done!
        try {
...
server.listen(WSport, () => {
  console.log(`Socket.IO server is running on port ${WSport}`);
});
